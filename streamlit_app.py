#!/usr/bin/env python3
"""
HIVE + QQQ + SOXS Trading Signal Dashboard.
...
def build_hive_data(period: str = "1y") -> pd.DataFrame:
    syms = {"HIVE": "HIVE", "BTC": "BTC-USD", "QQQ": "QQQ", "VIX": "^VIX"}
...
def build_soxs_data(period: str = "1y") -> pd.DataFrame:
    syms = {"QQQ": "QQQ", "SOXS": "SOXS", "VIX": "^VIX"}
    frames: dict[str, pd.Series] = {}
    for col, sym in syms.items():
        for attempt in range(3):
            try:
        ...
        if col not in frames:
            ...
            st.stop()
    df = pd.concat(frames.values(), axis=1).sort_index().ffill().dropna()
    df["QQQ_MA20"]    = df["QQQ"].rolling(QQQ_MA_SOXS_LOOKBACK).mean()
...
def classify_soxs(row: pd.Series) -> dict:
    ...
    base = {"qqq": qqq, "qqq_ma20": qma, "soxs": soxs,
            "soxs_high20": sh20, "soxs_pull": pull, "vix": vix}

    if vix >= SOXS_VIX_EMERGENCY:
        return {**base, "regime": "emergency",
                "action": "COVER / FLATTEN SOXS SHORT", "confidence": "high",
    ...
    if vix >= SOXS_VIX_DANGER:
        return {**base, "regime": "danger",
                "action": "REDUCE SOXS SHORT / DO NOT ADD", "confidence": "high",
    ...
    if vix >= SOXS_VIX_CAUTION:
        return {**base, "regime": "caution",
                "action": "HOLD SMALL SHORT; DO NOT ADD", "confidence": "medium",
                "target_short_alloc": SOXS_CAUTION_ALLOC,
                "reason": "VIX elevated. Hold a reduced short position; no new additions until VIX cools."}
    if at_high:
        if breaking_dn and not below_ma:
            return {**base, "regime": "soxs_spike",
                    "action": "START / ADD SMALL SOXS SHORT", "confidence": "medium",
                    "target_short_alloc": SOXS_CAUTION_ALLOC,
                    "reason": "SOXS spiked but is rolling over while QQQ holds. Early entry window open."}
        return {**base, "regime": "soxs_spike",
                "action": "WAIT — DO NOT SHORT VERTICAL SPIKE", "confidence": "high",
                "target_short_alloc": SOXS_CAUTION_ALLOC,
                "reason": "SOXS is near its 20-day high. Wait for a confirmed pullback before shorting."}
    if below_ma:
        return {**base, "regime": "qqq_soft",
                "action": "HOLD OR REDUCE; DO NOT ADD", "confidence": "medium",
                "target_short_alloc": SOXS_CAUTION_ALLOC,
    ...
    return {**base, "regime": "normal",
            "action": "SHORT / ADD SOXS TOWARD 50% TARGET", "confidence": "medium",
            "target_short_alloc": SOXS_NORMAL_ALLOC,
            "reason": f"VIX calm, QQQ above MA20, SOXS not at panic high — good risk/reward for short.{note}"}
...
def make_hive_chart(df: pd.DataFrame, regimes: pd.Series) -> go.Figure:
    ...
    _regime_bands(fig, regimes, HIVE_REGIME_COLORS, row=1)
    ...
    for ts, reg in transitions.items():
        ...
        fig.add_trace(go.Scatter(x=[ts], y=[df.loc[ts, "HIVE"]], mode="markers",
                                 showlegend=False,
    ...
    for y0, y1, c in [
        (0, VIX_CALM, "rgba(16,185,129,0.07)"),
        (VIX_CALM, VIX_CAUTION, "rgba(251,191,36,0.07)"),
        (VIX_CAUTION, VIX_DANGER, "rgba(249,115,22,0.08)"),
        (VIX_DANGER, 120, "rgba(220,38,38,0.10)"),
    ]:
    ...
    fig.update_yaxes(showgrid=True, gridcolor="rgba(107,114,128,0.10)", zeroline=False, color="#6b7280")
...
def make_soxs_chart(df: pd.DataFrame, regimes: pd.Series) -> go.Figure:
    ...
    _regime_bands(fig, regimes, SOXS_REGIME_COLORS, row=1)
    ...
    for y0, y1, c in [
        (0, SOXS_VIX_CAUTION, "rgba(16,185,129,0.07)"),
        (SOXS_VIX_CAUTION, SOXS_VIX_DANGER, "rgba(251,191,36,0.07)"),
        (SOXS_VIX_DANGER, SOXS_VIX_EMERGENCY, "rgba(249,115,22,0.08)"),
        (SOXS_VIX_EMERGENCY, 120, "rgba(220,38,38,0.10)"),
    ]:
        fig.add_hrect(y0=y0, y1=y1, fillcolor=c, layer="below", line_width=0, row=3, col=1)
    fig.add_trace(go.Scatter(x=df.index, y=df["VIX"], name="VIX",
                             line=dict(color="#f87171", width=1.5)), row=3, col=1)
...
def _hive_prompt(sig: dict, df: pd.DataFrame) -> str:
    hive_ret = _5d_ret(df["HIVE"])
    qqq_ret  = _5d_ret(df["QQQ"])
    return (
        ...
        f"VIX: {sig['vix']:.2f}\n"
        f"5-day HIVE: {hive_ret}  5-day QQQ: {qqq_ret}"
    )


def _soxs_prompt(sig: dict, df: pd.DataFrame) -> str:
    qqq_ret  = _5d_ret(df["QQQ"])
    soxs_ret = _5d_ret(df["SOXS"])
    return (
        "You are a concise trading signal analyst for a private dashboard.\n"
        ...
        f"VIX: {sig['vix']:.2f}\n"
        f"5-day QQQ: {qqq_ret}  5-day SOXS: {soxs_ret}"
    )
...
def main() -> None:
    ...
    st.sidebar.caption("HIVE + QQQ + SOXS overlay")
    ...
    tab_hive, tab_soxs = st.tabs(["🐝 HIVE + QQQ Core", "📉 SOXS Short Overlay"])
    ...
    with tab_hive:
        ...
        st.markdown(
            f'<div class="ts">Last bar: {df_h.index[-1].strftime("%Y-%m-%d %a")} '
            f'&nbsp;·&nbsp; Cache refreshes every 5 min</div>',
            unsafe_allow_html=True,
        )

        alloc_h = (
            f"QQQ {sig_h['qqq_target_alloc']:.0%}  ·  "
            f"HIVE {sig_h['hive_target_alloc']:.0%}  ·  "
            f"Cash {sig_h['target_cash_alloc']:.0%}"
        )
        ...
        badges_h: list[str] = [
            badge(f"BTC {'↑' if sig_h['btc_above_ma20'] else '↓'} MA20",
                  "g" if sig_h["btc_above_ma20"] else "r"),
            badge(f"QQQ {'↑' if sig_h['qqq_above_ma50'] else '↓'} MA50",
                  "g" if sig_h["qqq_above_ma50"] else "r"),
            badge(f"HIVE {'↑' if sig_h['hive_above_ma20'] else '↓'} MA20",
                  "g" if sig_h["hive_above_ma20"] else "r"),
            badge(
                f"VIX {sig_h['vix']:.1f}",
                "g" if sig_h["vix"] < VIX_CALM else
                ("a" if sig_h["vix"] < VIX_CAUTION else
                 ("r" if sig_h["vix"] < VIX_DANGER else "x")),
            ),
            badge(f"Conf: {sig_h['confidence'].upper()}",
                  "g" if sig_h["confidence"] == "high" else "a"),
        ]
        ...
        st.markdown("---")
        if st.button("🤖 Generate AI Brief", key="ai_hive"):
            with st.spinner("Thinking…"):
                brief_h = generate_ai_brief(_hive_prompt(sig_h, df_h))
            st.markdown(brief_h)
        ...
        col_dl.download_button(
            ...
            file_name=f"hive_qqq_{now_et.strftime('%Y%m%d')}.json",
    ...
    with tab_soxs:
        ...
        st.markdown(
            f'<div class="ts">Last bar: {df_s.index[-1].strftime("%Y-%m-%d %a")} '
            f'&nbsp;·&nbsp; Cache refreshes every 5 min</div>',
            unsafe_allow_html=True,
        )
        ...
        alloc_s = (
            f"QQQ Core 40%  ·  "
            f"SOXS Short {sig_s['target_short_alloc']:.0%}  ·  "
            f"Cash {max(0.0, 1.0 - 0.40 - sig_s['target_short_alloc']):.0%}"
        )
        hero_card(sig_s["regime"], sig_s["action"], alloc_s, sig_s["reason"], scolor)

        qqq_above = sig_s["qqq"] >= sig_s["qqq_ma20"]
        pull = sig_s["soxs_pull"]
        vix_s = sig_s["vix"]
        badges_s: list[str] = [
            badge(f"QQQ {'↑' if qqq_above else '↓'} MA20", "g" if qqq_above else "r"),
            badge(
                f"SOXS vs High20 {pull:+.1%}",
                "g" if pull <= -0.08 else ("a" if pull <= -0.03 else "r"),
            ),
            badge(
                f"VIX {vix_s:.1f}",
                "g" if vix_s < SOXS_VIX_CAUTION else
                ("a" if vix_s < SOXS_VIX_DANGER else
                 ("r" if vix_s < SOXS_VIX_EMERGENCY else "x")),
            ),
            badge(f"Conf: {sig_s['confidence'].upper()}",
                  "g" if sig_s["confidence"] == "high" else "a"),
        ]
        ...
        st.markdown("---")
        if st.button("🤖 Generate AI Brief", key="ai_soxs"):
            with st.spinner("Thinking…"):
                brief_s = generate_ai_brief(_soxs_prompt(sig_s, df_s))
            st.markdown(brief_s)
        ...
        col_dl.download_button(
            ...
            file_name=f"soxs_signal_{now_et.strftime('%Y%m%d')}.json",
            mime="application/json",
        )
